...
class Broadcaster:
  ...
  def broadcastMessage(self, message):
    users = db.GqlQuery("SELECT * FROM UserData where shard = :shard", shard = self.sending_user.shard)
    for user in users:
      if (datetime.datetime.now() - user.last_update_time > datetime.timedelta(hours = 1)):
  ...
  def getScores(self):
    ...
    for user in users:
      scores.append({'u' : user.display_name,
                     's': user.score,
                     't': len(Questions.questions) - len(user.available_questions) - 1})
    scores.sort()
    scores.reverse()
    return scores[:8]
...
class UserMessager:
    ...
    def sendNewQuestionToUser(self, user_data, opt_message = None):
      if (user_data != None):
        ...
        question_id = random.randint(0, len(Questions.questions) - 1)
      ...
      if (question_id >= 0):
        message['q'] = {
          'id': question_id,
          'q': Questions.questions[question_id]['q'],
          'a': Questions.questions[question_id]['a'][:]
        }
    ...
    def post(self):
      ...
      if user:
        ...
        if (user_data.available_questions.count(question_id) > 0):
          ...
          correct_answer = Questions.questions[question_id]['a'][0]
  ...
  def post(self):
    ...
    if user:
      user_data = UserData.get_or_insert(user.user_id(), user = user, score = 0,
                             available_questions = range(0, len(Questions.questions) - 1));
      messager = UserMessager(user.user_id())
...
class StartOverPage(webapp.RequestHandler):
  ...
  def post(self):
    ...
    if user:
      ...
      user_data.user = user
      user_data.score = 0
      user_data.available_questions = range(0, len(Questions.questions) - 1)
      db.put(user_data)
